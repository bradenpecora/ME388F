...
def streaming_update_numba(
    ...
    abs_polar_mu = abs(polar_mu)
    for i in range(n):
        mfp = sigma_t[i] * lengths[i]
        if mfp < machine_epsilon or abs_polar_mu < machine_epsilon:
            tau = 0.0
            exp_term = 1.0
            inv_2_sigma_t = 0.5 / sigma_t[i] if sigma_t[i] > 0 else 0.0
            source = (fixed_source[i] * sigma_t[i]) * inv_2_sigma_t
            exiting_angular_flux = incident_angular_flux + source
            average_angular_flux = (
                1 - 2 * tau
            ) * incident_angular_flux + 2 * tau * fixed_source[i] / sigma_t[i]
        else:
            tau = mfp / abs_polar_mu
            exp_term = np.exp(-tau)
            # Use the same formula as in your latest solve_ray_
            source = (
                ((1 - exp_term) / 2)
                * (fixed_source[i] + prior_flux[i] * sigma_s[i])
                / sigma_t[i]
            )
            exiting_angular_flux = incident_angular_flux * exp_term + source
            source_avg = (sigma_s[i] * prior_flux[i] + fixed_source[i]) / 2
            average_angular_flux = (
                incident_angular_flux - exiting_angular_flux
            ) / tau + source_avg / sigma_t[i]

        flux_contribution = 2.0 * (
            average_angular_flux
            * polar_weight
            * azimuthal_weight
            * lengths[i]
...
class CellData:
    def __init__(self, cell: shapely.geometry.Polygon, material: Material):
        ...
        self.area = cell.area
        self.fixed_source = material.volumetric_source
...
class CartesianMOC:
    ...
    def solve(self, tolerance=1e-6):
        ...
        for iteration in range(self.max_iterations):
            ...
            if error > past_error:
                raise Exception("Error increased!")
    ...
    def solve_ray_(
        ...
        polar_mus_weights = list(self.quadrature.polar_mus_weights())

        for polar_mu, polar_weight in polar_mus_weights:
            abs_polar_mu = np.abs(polar_mu)
            incident_angular_flux = incident_scalar_flux

            for cell_data, length in zip(intersecting_cell_datas, intersection_lengths):
                mat = cell_data.material
                sigma_s = mat.sigma_s
                sigma_t = mat.sigma_t
                prior_flux = cell_data.prior_flux
                fixed_source = cell_data.fixed_source
                ...
                if mfp < MACHINE_EPSILON or abs_polar_mu < MACHINE_EPSILON:
                    ...
                    # exp_term = 1.0
                    tau = mfp / abs_polar_mu
                    inv_2_sigma_t = 0.5 / sigma_t if sigma_t > 0 else 0.0

                    source = (fixed_source * sigma_t) * inv_2_sigma_t
                    exiting_angular_flux = incident_angular_flux + source
                    average_angular_flux = (
                        1 - 2 * tau
                    ) * incident_angular_flux + 2 * tau * fixed_source / sigma_t
                else:
                    tau = mfp / abs_polar_mu
                    exp_term = np.exp(-tau)
                    ...
                    # source = (
                    #     (1 - exp_term) * sigma_s * prior_flux + fixed_source * sigma_t
                    # ) * inv_2_sigma_t
                    source = (
                        ((1 - exp_term) / 2)
                        * (fixed_source + prior_flux * sigma_s)
                        / sigma_t
                    )
                    ...
                    # if tau == 0.0:
                    #     average_angular_flux = ((
                    #         sigma_s * prior_flux + fixed_source
                    #     ) * inv_2_sigma_t)
                    # else:
                    # average_angular_flux = (
                    #     (incident_angular_flux - exiting_angular_flux) / tau
                    #     + sigma_s * prior_flux * inv_2_sigma_t
                    #     + fixed_source * inv_2_sigma_t
                    # )
                    source = (sigma_s * prior_flux + fixed_source) / 2

                    average_angular_flux = (
                        incident_angular_flux - exiting_angular_flux
                    ) / tau + source / sigma_t

                # Optional: keep assertion for debugging
                # assert (
                #     np.isfinite(average_angular_flux) and average_angular_flux >= 0.0
                ...
                cell_data.add_flux(flux_contribution)
    ...
    def solve_ray(
        ...
        # Pre-extract all cell properties as arrays for Numba
        n = len(intersecting_cell_datas)
        sigma_t = np.array(
            [cell_data.material.sigma_t for cell_data in intersecting_cell_datas]
        )
        sigma_s = np.array(
            [cell_data.material.sigma_s for cell_data in intersecting_cell_datas]
        )
        prior_flux = np.array(
            [cell_data.prior_flux for cell_data in intersecting_cell_datas]
        )
        fixed_source = np.array(
            [cell_data.fixed_source for cell_data in intersecting_cell_datas]
        )
        lengths = np.array(intersection_lengths)

        for polar_mu, polar_weight in polar_mus_weights:
            # Call the Numba-accelerated streaming update
            for i, flux_contribution in streaming_update_numba(
                incident_scalar_flux,
                sigma_t,
                sigma_s,
                prior_flux,
    ...
    def extract_scalar_flux_on_line(self, x0, y0, x1, y1):
        """Extracts the scalar flux on a line from (x0, y0) to (x1, y1)."""
        line = shapely.geometry.LineString([(x0, y0), (x1, y1)])
        angle = np.arctan2(y1 - y0, x1 - x0)
        angle = normalize_angle(angle)

        intersecting_cells, intersecting_lengths = get_intersecting_cells(
            line, angle, self.tree
        )
        intersecting_cell_data = [
            self.cell_data_dict[cell] for cell in intersecting_cells
        ]
        scalar_fluxes = [cell.prior_flux for cell in intersecting_cell_data]
        positions = [cell.centroid for cell in intersecting_cells]
        x_pos = [pos.x for pos in positions]

        return x_pos, scalar_fluxes
